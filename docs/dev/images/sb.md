...
void main() {
    ...
    // Use a safe index: for example, take the sum of the coordinates and mod by 1024.
    uint index = (uint(coords.x + coords.y)) % 1024;
    float voxelFactor = voxels[index] / 255.0;
    ...
    vec4 debugColor = vec4(voxelFactor, voxelFactor, pc.fov, 1.0);

...
// Use raxel_coord_t as the chunk coordinate type.
typedef raxel_coord_t raxel_chunk_coord_t;

// Fix the enum definition (note: in your header there was a typo "tyepdef").
typedef enum raxel_voxel_chunk_state {
    RAXEL_VOXEL_CHUNK_STATE_COUNT = 0 // not used atm; you can later add more states
} raxel_voxel_chunk_state;
...
raxel_voxel_world_t *raxel_voxel_world_create(raxel_allocator_t *allocator) {
    ...
    world->chunks = raxel_list_create(raxel_voxel_chunk_t, allocator);
    world->chunk_meta = raxel_list_create(raxel_voxel_chunk_meta_t, allocator);
    world->__num_loaded_chunks = 0;
    world->materials = raxel_list_create(raxel_voxel_material_t, allocator);
    return world;
}
...
raxel_voxel_t raxel_world_get_voxel(raxel_voxel_world_t *world,
                                    ...
                                    raxel_coord_t z)
{
    raxel_chunk_coord_t chunk_x = x / RAXEL_VOXEL_CHUNK_SIZE;
    raxel_chunk_coord_t chunk_y = y / RAXEL_VOXEL_CHUNK_SIZE;
    raxel_chunk_coord_t chunk_z = z / RAXEL_VOXEL_CHUNK_SIZE;
                              ...
                              raxel_voxel_world_update_options_t *options)
{
    ...
    raxel_chunk_coord_t cam_chunk_x = options->camera_position[0] / RAXEL_VOXEL_CHUNK_SIZE;
    raxel_chunk_coord_t cam_chunk_y = options->camera_position[1] / RAXEL_VOXEL_CHUNK_SIZE;
    raxel_chunk_coord_t cam_chunk_z = options->camera_position[2] / RAXEL_VOXEL_CHUNK_SIZE;
